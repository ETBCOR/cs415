        ...
        for (j, j_pos) in positions.iter().enumerate() {
            if i != j
                && (i_pos.x == j_pos.x
                    ...
                    || i_pos.x - i_pos.y == j_pos.x - j_pos.y
                    || i_pos.y < 0 || i_pos.y >= NUMBER_OF_QUEENS
                    || j_pos.y < 0 || j_pos.y >= NUMBER_OF_QUEENS)
            {
                count += 1;
            }
        }
...
impl FitnessFunction<Positions, usize> for FitnessCalc {
    fn fitness_of(&self, positions: &Positions) -> usize {
        ...
        let max_collisions = (NUMBER_OF_QUEENS - 1) * (NUMBER_OF_QUEENS - 1) * 2;
    ...
    loop {
        ...
        match result {
            Ok(SimResult::Intermediate(step)) => {
                ...
                // }
                print!("      ");
                for p in best_solution.solution.genome {
                    print!("{:?}, ", p);
                }
                println!("");
            }
